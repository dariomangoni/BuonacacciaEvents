...
function GetBuonacacciaCompUpdates(){
    ...
    # $content | Out-File -FilePath .\buonacaccia_comp_mod.html
    ...
    {
        $match = $content[$cont_sel] -match '<a href="(?<link>event\.aspx\?e=\d+)">(?<base>.*?)\s*\|\s*(?<spec>.*?)\s*\|\s*(?<sex>\w+)</a></td>.*?(?<datestart>\d+/\d+/\d+).*?(?<dateend>\d+/\d+/\d+).*?MainContent_EventsGridView_Fee_\d+">(?<fee>.*?) \u20ac.*?MainContent_EventsGridView_Location_\d+".*?>(?<location>.*?) \((?<province>\w+)\).*?MainContent_EventsGridView_Taken_\d+">(?<taken>\d+).*\n*.*?MainContent_EventsGridView_SeatsMax_\d+">(?<seatsmax>\d+)'
        if ($match){
            $camp = @{}
    ...
    for ($camp_sel = 0; $camp_sel -lt $camps.count; $camp_sel++)
    {
        ...
        if ($new_availability){
            $new_aval_col = 'background-color:rgb(0, 255, 0)'
        }
        else {
            $new_aval_col = 'background-color:rgb(255, 255, 255)'
        }
        ...
        $newstr += '<tr>' +
                ...
                '<td style="text-align:center;">' + $camps[$camp_sel].sex + '</td>' +
                ...
                '<td style="text-align:left;">' + $camps[$camp_sel].base + '</td>' +
    ...
    Write-Output "Writing JSON archive file: .\buonacaccia_comp.json"
    # $ErrorActionPreference = "SilentlyContinue"
    $camps_json = $camps | ConvertTo-Json -Depth 4 -ErrorAction SilentlyContinue
    $camps_json | Out-File -FilePath .\buonacaccia_comp.json -Encoding UTF8
...
function GetBuonacacciaSpecUpdates(){
    ...
    $template = Get-Content -Path .\output_template_spec.html -Encoding UTF8 -Raw
    ...
    if ($camps_newstatus_str -ne ""){
        Write-Output "Writing HTML new events file: .\nuovi_eventi.html"
        $camps_newstatus_str = "<div>`n" + $camps_newstatus_str + "</div>`n"
        $camps_newstatus_str | Out-File -FilePath .\nuovi_eventi.html -Encoding UTF8 -Append
        return $true
    }
    else{
        return $false
